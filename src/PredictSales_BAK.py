...
# 커맨드라인에서 brandId 받기
if len(sys.argv) < 2:
    print("사용법: python PredictSales.py <brandId>")
    sys.exit(1)
brandId = sys.argv[1]
...
# 원하는 형식으로 출력
yyyymm  = '2025-08' #one_month_ago.strftime("%Y-%m")
print(yyyymm)
...
out_cols = [c for c in ["BRAND_ID","STORE_ID", 'YEAR', 'MONTH'
            ...
            if c in df_pred.columns] + ["PRED_SALES"]

print(df_pred[out_cols].head(20))  # 상위 20건 미리보기


# (선택) CSV로 저장
save_path = BASE_DIR / f"{brandId}_pred_sales_{yyyymm}.csv"
